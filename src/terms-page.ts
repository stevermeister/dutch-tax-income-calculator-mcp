...
<main>
  ...
  <div class="summary">
    <p><strong>In short:</strong> this is a free, indicative calculator, not tax advice, provided as-is with
    no warranty. Don't rely on it for filing or financial decisions without independent confirmation.</p>
  </div>

  <h2>The service</h2>
  <p>This server exposes Dutch salary/tax calculations — net-from-gross, gross-from-net, scenario comparisons,
  and official tax bracket lookups — over the Model Context Protocol, for use by AI assistants and their users.
  ...
  including accuracy, completeness, or fitness for a particular purpose. Tax rules change; a given tax year's
  brackets may be added, corrected, or become outdated between package updates.</p>

  <h2>Limitation of liability</h2>
  <p>To the maximum extent permitted by law, the operator of this server is not liable for any damages, losses,
  or costs arising from use of, or inability to use, this service, including reliance on any calculated result.</p>
  ...
  <p>Source code: <a href="${REPO_URL}" rel="noopener">${REPO_URL}</a>. Questions: open an issue at
  <a href="${REPO_ISSUES_URL}" rel="noopener">${REPO_ISSUES_URL}</a>. See also the
  <a href="${new URL(mcpUrl).origin}/mcp/privacy.html" rel="noopener">Privacy Policy</a>.</p>
</main>
</body>
</html>
`;
